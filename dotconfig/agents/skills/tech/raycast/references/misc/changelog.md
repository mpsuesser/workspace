...
* **Utils:** we’ve added a new React hook called `useLocalStorage`. This hook simplifies managing a value in `LocalStorage`. Take a look at the [developer docs](../utilities/react-hooks/uselocalstorage.md) to learn more.
...
* **Utils**: we’ve added a new React hook, `useStreamJSON`. The new hook simplifies the process of streaming through large JSON data sources, which normally would not fit in the extension’s memory. Take a look at the [developer docs](../utilities/react-hooks/usestreamjson.md) to learn more.
...
* `List` and `Grid` now have native pagination support! 🎉 If you want to update your extension to support pagination, head over to the [docs](../api-reference/user-interface/list.md#pagination) for instructions on how to get your extension to use pagination.
...
* **Arguments**: Introduced a new Argument type: `dropdown`. You can now [specify a list of options](../information/manifest.md#argument-properties) for the user choose from.
...
![](https://developers.raycast.com/files/XAvpR83ansTwHf6REDIf)
...
* Added [LaunchContext](../api-reference/command.md#launchcontext) support to `Create Quicklink` and `Create Snippet:`
...
* **Colors**: To improve accessibility, dynamic adjustment for raw colors (`HEX`, `rgb` etc) used in extensions has been switched from opt-in to opt-out. If your extension relies on accurate color reproduction, check the [documentation](../api-reference/user-interface/colors.md) for instructions on how to opt-out.
...
![](https://developers.raycast.com/files/MfwsgxrHINkI6Ia2ndaM)
...
* 🍫 **Menu Bar Commands (Beta)**: For a long time, Commands could only live in the Raycast window. From today, Commands can put glanceable information in the Menu Bar 💪. Check out our [new docs section](../api-reference/menu-bar-commands.md) on how to develop your first native macOS menu bar command with hot reloading 🔥.
* 🔄 **Background Refresh (Beta)**: To keep Menu Bar Commands up-to-date, we ported Background Refresh from [Script Commands](https://github.com/raycast/script-commands) to Extensions. Background Refresh is configured with a new interval option in the Extension's [manifest](../information/manifest.md) and also works for "no-view" mode commands. Read more about it in a [new docs guide](https://developers.raycast.com/information/background-refresh).
...
* 🤬 **Arguments**: We also ported more functionality from Script Commands. Extensions can now define arguments, which enable simple forms that live in the root search of Raycast. Arguments can be defined via the [manifest](../information/manifest.md), and their entered values are passed to the main function of a command.
...
* **Redesign**: Along side the app's redesign, we are introducing a whole set of [new icons](../api-reference/user-interface/icons-and-images.md#icon) for you to pick to illustrate the actions in your extensions.
...
* **DevTools**: The "Clear Local Storage" debug action has been renamed to "Clear Local Storage & Cache" and will clear the [Cache](../api-reference/cache.md) along side the [Local Storage](../api-reference/storage.md).
...
The `<Grid />` component's made its way to our API. It's perfect to layout media-heavy information, such as icons, images or colors. The component allows you to layout differently sized items. We designed [its API](../api-reference/user-interface/list.md) close to the `<List />` component for smooth adoption.

![](https://developers.raycast.com/files/S8GVBfkCcGY0xuhpuEoB)
...
* **OAuth**: we've added a new API that enables you to authorize extensions through OAuth providers such as Google, Twitter, Dropbox or Spotify ([![](https://www.notion.so/image/https%3A%2F%2Fwww.notion.so%2Fimages%2Fexternal_integrations%2Fgithub-icon.png?width=12\&userId=\&cache=v2)Issue #178](https://github.com/raycast/extensions/issues/178)). The docs contain a [new detailed guide](../api-reference/oauth.md) and we've added some integration examples to the extensions repository. (Note that we currently only support OAuth 2.0 with PKCE, more on that in the [guide](../api-reference/oauth.md).)
...
* **Fallback images**: You can now specify local asset files or built-in icons that are displayed when image loading fails, for example when a remote file is missing (![](https://www.notion.so/image/https%3A%2F%2Fwww.notion.so%2Fimages%2Fexternal_integrations%2Fgithub-icon.png?width=12\&userId=\&cache=v2)[Issue #108](https://github.com/raycast/extensions/issues/108)); [see the docs](../api-reference/user-interface/icons-and-images.md)
...
![](https://developers.raycast.com/files/o0MVOFVT9hOcWlh1U6sy)
