...
Oxlint already supports more than 700 rules from ESLint core and various popular plugins. We intend to support nearly all existing ESLint core rules, and this work is ongoing. Check the [compatibility matrix](./compatibility.md) to verify support for your frameworks and file types.
...
For further information on Oxlint's type-aware support, see [the Type-Aware Linting page](./type-aware.md).
...
For more information on JavaScript Plugins, see [the JS Plugins page](./js-plugins.md).
...
For plugins that are not implemented natively in Oxlint, it is recommended to use [JS Plugins](./js-plugins.md).
