...
const program = Effect.gen(function*() {
  ...
  const [inserted] = yield* sql`INSERT INTO users ${sql.insert({ name: "Alice" }).returning("*")}`
...
```ts
const UserLoaders = yield* SqlModel.makeDataLoaders(User, {
  tableName: "users",
...
const deleteResolver = SqlResolver.void({
  Request: UserId,
  execute: (ids) =>
    sql`DELETE FROM users WHERE ${sql.in("id", ids)}`
})
```
...
yield* sql`SELECT * FROM users`.pipe(
