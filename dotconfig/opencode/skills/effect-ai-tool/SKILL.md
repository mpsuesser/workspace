...
For comprehensive Effect AI documentation, view the Effect v4 repository at `.references/effect-v4/packages/ai/`
...
- **Note**: `Tool.fromTaggedRequest` may not be available in v4. `Tool.make` is the primary constructor — prefer it for new code.
...
class WeatherService extends ServiceMap.Service<WeatherService, {
...
import * as Toolkit from "effect/unstable/ai/Toolkit"
import * as Prompt from "effect/unstable/ai/Prompt"
import { Schema, Effect, Data, ServiceMap, Layer } from "effect"
...
2. **Tool.fromTaggedRequest** - Create tools from TaggedRequest schemas (may not be available in v4; prefer Tool.make)
3. **Parameters** - Accept field objects or Schema.Struct (both work)
