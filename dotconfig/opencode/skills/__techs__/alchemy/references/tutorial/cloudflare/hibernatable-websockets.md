...
The integration test connects to the deployed Worker over a real WebSocket using Effect's native [`Socket`](https://effect.website/docs/platform/socket/) module. Unlike the raw `WebSocket` global, `Socket` is fully Effect-aware: the connection lifecycle is tied to a `Scope`, incoming frames flow through a `Queue`, and the open handshake is signalled with a `Deferred`.
...
`Socket.makeWebSocket(url)` builds a `Socket` resource. `socket.writer` returns a function for sending frames. `socket.runString` consumes inbound text frames; we offer each to an unbounded queue. `Effect.forkScoped` runs the read loop in the background tied to the surrounding scope — when the test scope closes, the loop is interrupted and the socket is closed. `Deferred.await(opened)` blocks until `onOpen` fires so the caller can `send` immediately without racing the handshake.

## Write the test body

Open two clients to the same room, have one send a message, and assert the other receives the broadcast. Wrap the whole effect in `Effect.scoped`:
...
Verify it with a test that sends `/remind 1 hello` and waits for the broadcast:
...
Next: [add a Vite SPA](./vite-spa.md) so users have a UI to talk to it — frontend and backend deploying together from the same `alchemy.run.ts`.
