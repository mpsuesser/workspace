...
Every [Resource](./resource.md) moves through the same lifecycle: **plan → reconcile → (replace) → delete**. The plan classifies each resource as create, update, replace, delete, or no-op, but the provider implements a single `reconcile` function that converges the cloud's actual state to what's declared — whether that's the first provisioning, a routine update, or an adoption takeover.
...
The classification comes from each provider's `diff` function. See [Provider › diff](./provider.md#diff).
...
`provider.delete` is called when a resource disappears from your code, when a replacement supersedes it, or when you run `alchemy destroy`. Like reconcile, delete must be **idempotent**: deleting an already-gone resource is a success, not an error.
...
- **Adoption** — you're deploying against existing infrastructure you didn't manage with Alchemy yet (or you wiped state intentionally). `read` recognizes the resource and the engine imports it.

Providers signal "this is mine" vs "this exists but isn't mine" via the `Unowned(attrs)` brand:
...
See [Provider › read](./provider.md#read) for the implementation contract.
...
See the [CLI reference](../guides/cli.md) for the full set of flags (`--yes`, `--force`, `--dry-run`, `--stage`, `--profile`, …).
