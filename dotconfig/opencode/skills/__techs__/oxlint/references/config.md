...
For a complete list of fields, see the [Config file reference](./config-file-reference.md).

## Configure linter options

Use `options` for linter-level behavior. See the [Config file reference](./config-file-reference.md#options) for the full list.
...
All available rules, and their configuration options, are listed in the [Rules reference](./rules.md).
...
Oxlint supports many popular plugins natively in Rust. This provides broad rule coverage without a large JavaScript dependency tree. See [Native Plugins](./plugins.md).
...
For plugin details and CLI flags such as `--import-plugin`, see [Native Plugins](./plugins.md).
...
For details, see [JS plugins](./js-plugins.md).
...
- [Ignore files](./ignore-files.md): Ignore files and patterns, `.gitignore` and `.eslintignore` workflows, and symlink behavior.
- [Inline ignore comments](./ignore-comments.md): Inline suppressions and scoped exceptions.
