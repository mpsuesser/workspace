void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Random glitch block
    float glitchBlock = step(0.95, fract(sin(glitchTime * 57.0)));
    ...
    // Channel shifting
    vec3 rChannel = texture(iChannel0, glitchUV + vec2(0.01 * glitchBlock, 0.0)).r * vec3(1.0, 0.0, 0.0);
    vec3 gChannel = texture(iChannel0, glitchUV).g * vec3(0.0, 1.0, 0.0);
    vec3 bChannel = texture(iChannel0, glitchUV - vec2(0.01 * glitchBlock, 0.0)).b * vec3(0.0, 0.0, 1.0);
