...
export const registerCompactVccRecallCommand = (pi: ExtensionAPI) => {
  pi.registerCommand("compact-vcc-recall", {
    description: "Search conversation history (same as vcc_recall tool). Usage: /compact-vcc-recall <query> [page:N]",
    handler: async (args: string, ctx) => {
      ...
      const footer = page < totalPages
        ? `\n--- /compact-vcc-recall ${query} page:${page + 1} ---`
        : "";
