...
Start with the smallest possible subscription: log just the event name (`INSERT` / `MODIFY` / `REMOVE`) for each change:
...
The Lambda-specific implementation of `TableEventSource` is in `Lambda.TableEventSource`:
...
You'll see three log lines stream past — one per write. Stream records typically arrive within 1–2 seconds of the originating change.
