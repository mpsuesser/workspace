...
In [Part 4](./part-4.md) you ran your stack locally. Now you'll hand it off to GitHub Actions: pushes to `main` deploy to production, pull requests get isolated preview environments, and merged PRs clean up after themselves.
...
CI needs to share state across runs, so a local `.alchemy/` directory won't cut it. You already configured `Cloudflare.state()` back in [Part 1](./part-1.md), which deploys a Worker backed by a Durable Object with SQLite. Every deploy — local or from CI — reads and writes state remotely through that Worker.
...
- Go deeper on Cloudflare in the [Cloudflare track](./cloudflare/durable-objects.md) — layer Durable Objects, hibernatable WebSockets, Containers, Workflows, and Queues onto the Worker you built
- Read the [Resource Lifecycle](../concepts/resource-lifecycle.md) guide for all CLI flags and options
- Explore the [CI guide](../guides/ci.md) for the AWS equivalents (OIDC and access keys) and more workflow patterns
- Check out the [Testing reference](../concepts/testing.md) for more advanced testing patterns
