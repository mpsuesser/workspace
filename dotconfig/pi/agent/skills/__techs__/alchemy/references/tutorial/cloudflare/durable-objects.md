...
By the end of [Part 5](../part-5.md) you have a Worker deploying to multiple stages from CI. From here, the Cloudflare track adds one stateful primitive at a time. First up: a `Counter` Durable Object that keeps a per-key count in transactional storage, exposes RPC methods, and streams a sequence of numbers back to the caller.
...
Next you'll teach a Durable Object to [accept WebSocket connections](./hibernatable-websockets.md) — and learn how Cloudflare hibernates idle DOs while keeping connections alive.
