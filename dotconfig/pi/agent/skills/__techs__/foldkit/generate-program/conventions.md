...
> **Note:** This file is a snapshot of the conventions practiced in the live Foldkit codebase. If anything here contradicts what `~/.cache/foldkit/examples/` or `~/.cache/foldkit/packages/foldkit/src/` actually do, the live code is canonical. The duplication is intentional so the skill still works when the cached checkout is unavailable.
