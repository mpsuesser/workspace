...
- `~/.cache/foldkit/packages/typing-game/client/src/`: production multi-page app: Submodels, OutMessage, update/view decomposition, curried handler extraction, subscription patterns, domain modules.
- `~/.cache/foldkit/packages/website/src/`: production Foldkit website: page organization, shared view primitives, route-driven rendering, idiomatic domain separation.
...
Read `~/.cache/foldkit/examples/counter/src/main.ts`

**Tier 2: Timers, subscriptions, simple stateful apps:**
Read `~/.cache/foldkit/examples/stopwatch/src/main.ts` (timer via subscription, `Duration` field pattern) and `~/.cache/foldkit/examples/todo/src/main.ts` (CRUD with localStorage via flags)

**Tier 3: Async operations, loading/error states, API calls, form validation:**
Read `~/.cache/foldkit/examples/weather/src/main.ts` (HTTP with `HttpClient`) and `~/.cache/foldkit/examples/form/src/main.ts` (uses `foldkit/fieldValidation`; see the Form Validation section in Phase 4)

**Tier 4: URL routing, multiple pages, query parameters:**
Read `~/.cache/foldkit/examples/routing/src/main.ts` and `~/.cache/foldkit/examples/query-sync/src/main.ts`

**Tier 5: Complex state, nested domain models, CRUD, drag-and-drop:**
Read `~/.cache/foldkit/examples/shopping-cart/src/main.ts` (nested domain schemas, cart state) and `~/.cache/foldkit/examples/kanban/src/main.ts` (CRUD with `Ui.DragAndDrop`, flags restoring from localStorage, subscriptions)

**Tier 6: Submodels, OutMessage, multi-step forms, auth flows, multi-module apps:**
Read `~/.cache/foldkit/examples/auth/src/main.ts` (login/signup with Submodels, OutMessage, protected routes) and `~/.cache/foldkit/examples/job-application/src/main.ts` (multi-step form with deeply nested Submodels in `step/`, `Ui.DatePicker`, `Ui.FileDrop`, `Ui.Menu`, `Calendar` module for date handling)

**Tier 7: Real-time, WebSocket, managed resources, production-grade:**
Read `~/.cache/foldkit/packages/typing-game/client/src/update.ts`, then explore its `page/home/` and `page/room/` directories for the full Submodel/OutMessage pattern.
...
- `~/.cache/foldkit/examples/ui-showcase/src/main.ts`: root wiring, `Got*` delegation, `toParentMessage` helpers
- `~/.cache/foldkit/examples/ui-showcase/src/message.ts`: how UI component Messages are structured
- `~/.cache/foldkit/examples/ui-showcase/src/model.ts`: how UI component Models are composed
- `~/.cache/foldkit/examples/ui-showcase/src/update.ts`: how UI component updates are delegated
- `~/.cache/foldkit/examples/ui-showcase/src/toast.ts`: read when using `Ui.Toast`: Toast is unique in that it's parameterized on a payload schema via `Ui.Toast.make(PayloadSchema)`, returning a typed module you import from
...
### Ensure the Foldkit cache exists

After scaffolding, ensure the shared Foldkit checkout exists at `~/.cache/foldkit/` so future AI sessions can reference the full source, examples, and docs without vendoring a copy into each generated project:

```bash
if [ -d ~/.cache/foldkit/.git ]; then
  git -C ~/.cache/foldkit pull --ff-only
else
  mkdir -p ~/.cache
...
This is optional but strongly recommended. The cached checkout gives agents full local context without modifying the user's project tree. Do not offer to add a project-local Foldkit subtree unless the user explicitly asks for one.

To refresh the cache later: `git -C ~/.cache/foldkit pull --ff-only`.
...
- For apps with multiple domain entities referenced across modules, extract shared schemas into `src/domain/` (e.g., `domain/product.ts`, `domain/session.ts`). See the shopping-cart and auth examples for this pattern, and read `~/.cache/foldkit/packages/website/src/page/projectOrganization.ts` for guidance on when and how to structure domain modules
...
Canonical reference: `~/.cache/foldkit/examples/form/src/main.ts` (async email uniqueness check with version-based cancellation) and `~/.cache/foldkit/examples/job-application/src/step/` (validated multi-step forms across submodels).
...
- Canonical reference: `~/.cache/foldkit/examples/job-application/src/step/attachments.ts`
...
- `~/.cache/foldkit/examples/weather/src/main.story.test.ts`: simple Command resolution (happy path + error path)
- `~/.cache/foldkit/examples/auth/src/page/loggedOut/page/login.story.test.ts`: Submodel with OutMessage assertions, field validation
- `~/.cache/foldkit/packages/website/src/search/update.test.ts`: multi-step interactions (arrow key cycling, stale result handling)
...
- `~/.cache/foldkit/examples/weather/src/main.scene.test.ts`: basic Scene flow with form interaction and Command resolution
- `~/.cache/foldkit/examples/auth/src/page/loggedOut/page/login.scene.test.ts`: Submodel Scene testing, field validation through the view
- `~/.cache/foldkit/examples/kanban/src/main.scene.test.ts`: scoped queries with `within`, `toHaveValue`, explicit test data
...
code in `~/.cache/foldkit/packages/typing-game/client/src/` or `~/.cache/foldkit/packages/website/src/`.
Not "works." Not "structurally valid." Typing-game quality.

Read FIRST, in this order:
1. <absolute path to generated src/ files: list every file>
2. ~/.cache/foldkit/skills/generate-program/architecture.md
3. ~/.cache/foldkit/skills/generate-program/conventions.md
4. ~/.cache/foldkit/skills/generate-program/checklist.md
5. At least one exemplar file matching the generated app's complexity:
   - Tier 1-2: ~/.cache/foldkit/packages/foldkit/src/runtime/runtime.ts (for quality calibration)
   - Tier 3-4: ~/.cache/foldkit/examples/weather/src/main.ts OR ~/.cache/foldkit/examples/form/src/main.ts
   - Tier 5: ~/.cache/foldkit/examples/kanban/src/update.ts AND ~/.cache/foldkit/examples/kanban/src/domain/
   - Tier 6: ~/.cache/foldkit/examples/job-application/src/update.ts AND ~/.cache/foldkit/examples/auth/src/page/
