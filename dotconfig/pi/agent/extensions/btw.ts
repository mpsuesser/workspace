...
): Promise<SideChat | null>
{
	...
	const systemPrompt =
		`This is a side conversation ("by the way") from a main coding session.
...
class SideChatOverlay implements Focusable
{
	...
	handleInput(data: string): void
	{
		...
		if (matchesKey(data, 'backspace'))
		{
			...
			{
				this.inputText = this.inputText.slice(0, this.inputCursor - 1)
		...
		{
			this.inputCursor = Math.min(this.inputText.length,
				this.inputCursor + 1);
		}
		else if (data.length === 1 && data.charCodeAt(0) >= 32)
		{
			this.inputText = this.inputText.slice(0, this.inputCursor) + data
				+ this.inputText.slice(this.inputCursor);
			this.inputCursor++;
		}
	}
	...
	render(_width: number): string[]
	{
		...
		const row = (content: string) =>
			` ${th.fg('border', '│')} ` + pad(content, innerW)
			+ ` ${th.fg('border', '│')}`;

		lines.push(` ${th.fg('border', `╭${'─'.repeat(innerW)}╮`)}`);
		lines.push(
			row(`${th.fg('accent', '🔄 BTW Side Chat')} ${
				th.fg('dim', `| ${this.chat.id}`)
			}`),
		);
		lines.push(
			row(`${th.fg('dim', 'Topic: ')}${
				truncateToWidth(this.chat.topic, innerW - 7)
			}`),
		);
		lines.push(
			row(th.fg('dim',
				`Started: ${
					formatTime(this.chat.startTime)
				} | ${this.chat.messages.length} msgs`)),
		);
		lines.push(` ${th.fg('border', `├${'─'.repeat(innerW)}┤`)}`);
		...
		for (let i = 0; i < contentH; i++)
		{
			...
			const line = prefix
				+ th.fg('text', truncateToWidth(content, innerW - 5));
			lines.push(row(line));
		}
		...
		if (this.chat.status === 'error')
		{
			status = th.fg('error',
				`Error: ${
					truncateToWidth(this.chat.error || 'Unknown', innerW - 7)
				}`);
		}
		...
		if (this.focused && this.inputCursor < inputDisplay.length)
		{
			...
			inputDisplay = before + CURSOR_MARKER + `\x1b[7m${at}\x1b[27m`
				+ after;
		}
		...
		lines.push(` ${th.fg('border', `├${'─'.repeat(innerW)}┤`)}`);
		lines.push(
			row(th.fg('dim', 'Enter to send | Esc to hide | Ctrl+C to close')),
		);
		lines.push(` ${th.fg('border', `╰${'─'.repeat(innerW)}╯`)}`);
...
export default function(pi: ExtensionAPI)
{
	...
	pi.registerCommand('btw', {
		...
		handler: async (args, ctx) =>
		{
			...
			const chat = await createSideChat(id, topic, contextMessages,
				ctx.cwd, (c, role, content) =>
			{
				if (role === 'assistant')
				{
	...
	pi.registerCommand('btw-list', {
		...
		handler: async (_args, ctx) =>
		{
			...
			{
				ctx.ui.notify('No active side chats. Use /btw to start one.',
					'info');
				return;
			}
			...
			for (const [id, chat] of activeChats)
			{
				const icon = chat.status === 'running'
					? '🔄'
					: chat.status === 'error'
					? '❌'
					: '✓';
	...
	pi.registerCommand('btw-resume', {
		...
		handler: async (args, ctx) =>
		{
			...
			if (!id)
			{
				...
				{
					ctx.ui.notify(
						'No running side chats. Use /btw-list to see all.',
						'warning',
					);
					return;
				}
