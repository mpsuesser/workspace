...
### Added
- Auto-promote a top-level **foreground** run to **async** when its effective dispatched agents can call `contact_supervisor` and the intercom bridge is active. This now checks the post-bridge child toolset, so bridge-injected `contact_supervisor` on agents such as `planner`/`reviewer` is covered, and explicit `async: false` or `clarify: true` no longer bypasses the safety rule. A foreground run blocks the orchestrator inside the tool call, so a child that calls `contact_supervisor({ reason: "need_decision" })` can never get an answer until the run returns and instead hangs until pi-intercom's 10-minute reply timeout — and the decision is never made. Running async keeps the orchestrator free to answer, so coordination actually works. The promoted run returns a one-line note explaining the switch. Opt out only globally with `autoPromoteSupervisorRunsToAsync: false` if you intentionally accept the deadlock risk.

### Fixed
- Avoid duplicate model wake-ups for async completions when grouped intercom result delivery succeeds. `subagent-notify` now remains a fallback for missing/unacknowledged intercom delivery instead of competing with the richer grouped result, paused interrupt copy no longer asks for an "explicit next action", and resume hints are worded as optional follow-ups.
- Preserve `paused` in async job tracking when a completion event reports an interrupted run, instead of collapsing every unsuccessful completion to `failed`.
- Fail closed instead of launching a deadlock-prone top-level foreground run when the effective child can block on `contact_supervisor` but async execution is unavailable.
- Stopped async control notices from suggesting generic `intercom({ action: "send" })` nudges to busy headless child sessions. Async children run non-interactively while working and cannot read freeform intercom messages until they exit, so notices now recommend status inspection, waiting if progress is advancing, and interrupting only when clearly stuck.
- Reduced noisy async attention defaults: idle `needs_attention` now waits five minutes by default (`300000` ms), and control notices default to direct parent events plus async persistence instead of also sending a duplicate control-intercom ping. Explicit `intercom` control delivery remains available, and `completion_guard` failures still surface.
- Cleared stale idle `needs_attention` status when later child activity is observed, without clearing real failure signals such as `completion_guard` or repeated mutating-tool failures.
...
### Changed
- Moved the optional extension config file from `~/.pi/agent/extensions/subagent/config.json` to `~/.pi/agent/subagent-config.json`.
