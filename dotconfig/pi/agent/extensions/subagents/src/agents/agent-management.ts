...
export function handleCreate(params: ManagementParams, ctx: ManagementContext): AgentToolResult<Details> {
	...
	if (!cfg) return result(
		"`action: 'create'` writes a NEW agent or chain definition file and requires a nested `config: { name, description, systemPrompt, ... }` object.\n" +
		"If you wanted to LAUNCH a run, do not use `action`. Use one of: { agent, task } | { tasks: [{agent, task}, ...] } | { chain: [...] }.",
		true,
	);
...
export function handleUpdate(params: ManagementParams, ctx: ManagementContext): AgentToolResult<Details> {
	...
	const cfg = parsedConfig.value;
	if (!cfg) return result(
		"`action: 'update'` modifies an existing agent or chain definition file and requires a nested `config: { ... }` object describing the merge.\n" +
		"If you wanted to LAUNCH a run, do not use `action`. Use one of: { agent, task } | { tasks: [{agent, task}, ...] } | { chain: [...] }.",
		true,
	);
	const warnings: string[] = [];
