...
export function executeAsyncChain(
	...
	const buildSeqStep = (s: SequentialStep, sessionFile?: string, behaviorCwd?: string, progressPrecreated = false, resolvedBehavior?: ResolvedStepBehavior) => {
		...
		const behaviorReadCwd = resultMode === "chain" ? chainDir : instructionCwd;
		const readInstructions = buildChainInstructions({ ...behavior, output: false, progress: false }, behaviorReadCwd, false);
		...
		const outputBaseCwd = resultMode === "chain" ? chainDir : instructionCwd;
		const outputPath = resolveSingleOutputPath(behavior.output, ctx.cwd, outputBaseCwd);
		...
		return {
			...
			inheritAvailableSkills: a.inheritAvailableSkills,
				...
				steps: [
					{
						...
						inheritAvailableSkills: agentConfig.inheritAvailableSkills,
						skills: resolvedSkills.map((r) => r.name),
						outputPath,
						outputMode,
						sessionFile,
