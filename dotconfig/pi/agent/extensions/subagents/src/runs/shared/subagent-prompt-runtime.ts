...
const SUBAGENT_INHERIT_AVAILABLE_SKILLS_ENV = "PI_SUBAGENT_INHERIT_AVAILABLE_SKILLS";
...
export const CHILD_SUBAGENT_BOUNDARY_INSTRUCTIONS = [
	...
	"The parent session owns delegation, orchestration, review fanout, and follow-up implementation launches.",
...
export function rewriteSubagentPrompt(
	prompt: string,
	options: { inheritProjectContext: boolean; inheritAvailableSkills: boolean; fanoutChild?: boolean },
): string {
	let rewritten = prompt;
	if (!options.inheritProjectContext) {
		rewritten = stripProjectContext(rewritten);
	}
	if (!options.inheritAvailableSkills) {
...
function isSignedThinkingBlock(block: unknown): boolean {
	const b = block as { type?: string; thinkingSignature?: unknown; signature?: unknown; redacted?: unknown; data?: unknown };
	if (b?.type === "redacted_thinking") return true;
	if (b?.type !== "thinking") return false;
	return b.redacted === true
		|| (typeof b.thinkingSignature === "string" && b.thinkingSignature.length > 0)
		|| (typeof b.signature === "string" && b.signature.length > 0)
		|| (typeof b.data === "string" && b.data.length > 0);
}

function assistantHasSignedThinking(message: { role?: string; content?: unknown }): boolean {
	return message.role === "assistant"
		&& Array.isArray(message.content)
		&& message.content.some(isSignedThinkingBlock);
}

function collectToolCallIds(message: { role?: string; content?: unknown }): string[] {
	if (message.role !== "assistant" || !Array.isArray(message.content)) return [];
	return message.content.flatMap((block) => {
		const b = block as { type?: string; id?: unknown };
		return b.type === "toolCall" && typeof b.id === "string" ? [b.id] : [];
	});
}
...
function stripAssistantSubagentToolCallBlocks(message: unknown): unknown | undefined {
	...
	// Anthropic signs thinking blocks against their original assistant message.
	// Never rewrite a signed-thinking message in place; drop the whole parent-only
	// turn instead and let the caller remove its matching tool results.
	if (assistantHasSignedThinking(m)) return undefined;
...
export function stripParentOnlySubagentMessages(messages: unknown[]): unknown[] {
	...
	let droppedAssistantToolResultIds = new Set<string>();
	for (const message of messages) {
		const m = message as { role?: string; toolCallId?: unknown; content?: unknown; customType?: string; toolName?: string };
		if (droppedAssistantToolResultIds.size > 0) {
			if (m?.role === "toolResult" && typeof m.toolCallId === "string" && droppedAssistantToolResultIds.has(m.toolCallId)) {
				changed = true;
				continue;
			}
			if (m?.role !== "toolResult") {
				droppedAssistantToolResultIds = new Set<string>();
			}
		}
		...
		if (stripped === undefined) {
			changed = true;
			if (m?.role === "assistant" && Array.isArray(m.content) && m.content.some(isSubagentToolCallBlock)) {
				droppedAssistantToolResultIds = new Set(collectToolCallIds(m));
			}
			continue;
		}
...
export default function registerSubagentPromptRuntime(pi: ExtensionAPI): void {
	...
	onRuntimeEvent("before_agent_start", async (event: { systemPrompt: string }) => {
		...
		const inheritAvailableSkills = readBooleanEnv(SUBAGENT_INHERIT_AVAILABLE_SKILLS_ENV);
		const fanoutChild = readBooleanEnv(SUBAGENT_FANOUT_CHILD_ENV);
		if (inheritProjectContext === undefined && inheritAvailableSkills === undefined && fanoutChild === undefined) return;
		const rewritten = rewriteSubagentPrompt(event.systemPrompt, {
			inheritProjectContext: inheritProjectContext ?? true,
			inheritAvailableSkills: inheritAvailableSkills ?? true,
