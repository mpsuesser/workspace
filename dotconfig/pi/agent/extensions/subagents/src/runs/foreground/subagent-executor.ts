...
import { normalizeStringifiedArrayParams } from "../../shared/normalize-array-params.ts";
...
async function runParallelPath(data: ExecutionContextData, deps: ExecutorDeps): Promise<AgentToolResult<Details>> {
	...
	try {
		...
		const parallelProgressDir = path.join(TEMP_ROOT_DIR, "progress", runId);
...
export function createSubagentExecutor(deps: ExecutorDeps): {
	...
	const execute = async (
		...
		// Normalize string-form `tasks` / `chain` (auto-parse JSON arrays). This rescues
		// tool calls from providers (e.g. openai-codex) that stringify nested array args.
		const normalizedArrayParams = normalizeStringifiedArrayParams(params as unknown as Record<string, unknown>);
		if (!normalizedArrayParams.ok) {
			return {
				content: [{ type: "text", text: normalizedArrayParams.error }],
				isError: true,
				details: { mode: "management", results: [] },
			};
		}
		params = normalizedArrayParams.params as unknown as SubagentParamsLike;
