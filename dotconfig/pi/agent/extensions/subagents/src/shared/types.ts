...
export interface Details {
	...
	chainAgents?: string[];      // Agent names in order, e.g., ["agent-a", "agent-b"]
...
export function resolvePiEphemeralDir(options?: {
	...
	const configured = env.PI_SUBAGENT_EPHEMERAL_DIR ?? env.PI_EPHEMERAL_DIR;
	if (configured) return path.resolve(expandHomePath(configured, resolveHome));
	const agentDir = path.resolve(expandHomePath(env.PI_CODING_AGENT_DIR ?? path.join(resolveHome(), ".pi", "agent"), resolveHome));
	return path.join(path.dirname(agentDir), "ephemeral");
}
...
export const PI_EPHEMERAL_DIR = resolvePiEphemeralDir();
export const TEMP_ROOT_DIR = path.join(PI_EPHEMERAL_DIR, `pi-subagents-${resolveTempScopeId()}`);
