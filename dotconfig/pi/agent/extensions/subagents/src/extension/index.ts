...
export default function registerSubagentExtension(pi: ExtensionAPI): void {
	...
	const tool: ToolDefinition<typeof SubagentParams, Details> = {
		name: "subagent",
		label: "Subagent",
		promptSnippet: "Delegate to subagents to RUN work: { agent, task } single, { tasks: [{agent, task}, ...] } parallel, or { chain: [...] } pipeline. The launch-a-run forms have NO `action` field. `action: \"create\"` is ONLY for defining a new agent (needs `config: {name, description, systemPrompt, ...}`) — never use it to start a run. Start with { action: \"list\" } to see available agents.",
		description: `Delegate to subagents (run work) or manage agent definitions.

LAUNCHING RUNS — these forms have NO action field. Use exactly ONE shape:
• SINGLE: { agent: "name", task: "..." } — one task; omit task for self-contained agents
• PARALLEL: { tasks: [{agent,task,count?,output?,reads?,progress?}, ...], concurrency?: number, worktree?: true } — concurrent execution. The literal field names are 'agent' and 'task'.
• CHAIN: { chain: [{agent:"agent-a"}, {parallel:[{agent:"agent-b",count:3}]}] } — sequential pipeline with optional fan-out

Field names on task items are literal:
• "agent" (NOT "name"). The runtime agent name; for packaged agents use "package.agent".
• "task" (NOT "prompt", NOT "title", NOT "description"). The instructions string passed to the child.

If the provider stringifies the array (some openai-codex tool callers do this), the tool auto-parses string-form 'tasks' and 'chain' — but prefer emitting real JSON arrays.
...
Before launching unfamiliar work, use { action: "list" } to inspect configured agents/chains. Only execute agents listed as executable/non-disabled.

CHAIN TEMPLATE VARIABLES (use in task strings):
• {task} — The original task/request from the user
• {previous} — Text response from the previous step (empty for first step)
• {chain_dir} — Shared directory for chain files under the Pi ephemeral state area (e.g., /Users/m/repos/workspace/dotconfig/pi/ephemeral/pi-subagents-<scope>/chain-runs/abc123/)

Ephemeral handoff/scratch files (plans, context reports, progress notes, research briefs) should go under /Users/m/repos/workspace/dotconfig/pi/ephemeral/ with descriptive unique names. Do not direct children to create cwd-level canonical files such as context.md, plan.md, report.md, or progress.md unless the user explicitly asks for that path.

Example: { chain: [{agent:"agent-a", task:"Analyze {task}"}, {agent:"agent-b", task:"Plan based on {previous}"}] }

MANAGING AGENT DEFINITIONS — use the action field. These do NOT launch runs:
• { action: "list" } — discover executable agents/chains
• { action: "get", agent: "name" } — full detail; packaged agents use dotted runtime names like "package.agent"
• { action: "create", config: { name, description, systemPrompt, ... } } — writes a NEW agent or chain definition file. Requires the nested config object. NOT for starting a run; to start runs use the launch forms above.
• { action: "update", agent: "code-analysis.custom-agent", config: { ... } } — merge into an existing definition
