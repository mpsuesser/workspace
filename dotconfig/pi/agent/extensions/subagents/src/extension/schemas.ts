...
const TaskItem = Type.Object({
	...
	progress: Type.Optional(Type.Boolean({ description: "Enable progress tracking for this task using a runtime-provided scratch path" })),
...
const ParallelTaskSchema = Type.Object({
	...
	outputMode: Type.Optional(OutputModeOverride),
	reads: Type.Optional(ReadsOverride),
	progress: Type.Optional(Type.Boolean({ description: "Enable progress tracking in {chain_dir}" })),
	skill: Type.Optional(SkillOverride),
...
const ControlOverrides = Type.Object({
	...
	needsAttentionAfterMs: Type.Optional(Type.Integer({ minimum: 1, description: "No-observed-activity window before a run needs attention (default: 300000)" })),
	...
	notifyChannels: Type.Optional(Type.Array(Type.String({ enum: ["event", "async", "intercom"] }), {
		description: "Notification channels to use when available. Defaults to event and async; intercom is opt-in for control notices.",
	})),
});
...
export const SubagentParams = Type.Object({
	...
	config: Type.Optional(Type.Unsafe({
		...
		description: "Agent or chain config for create/update. Agent: name, package (optional namespace; runtime name becomes package.name), description, scope ('user'|'project', default 'user'), systemPrompt, systemPromptMode, inheritProjectContext, inheritAvailableSkills, defaultContext ('fresh'|'fork'), model, tools (comma-separated), extensions (comma-separated), skills (comma-separated), thinking, output, reads, progress, maxSubagentDepth. Chain: name, package, description, scope, steps (array of {agent, task?, output?, outputMode?, reads?, model?, skill?, progress?}). Presence of 'steps' creates a chain instead of an agent. String values must be valid JSON."
	})),
	tasks: Type.Optional(Type.Unsafe({
		anyOf: [
			Type.Array(TaskItem),
			{ type: "string", description: "JSON-encoded array of task items; auto-parsed before execution. Provided for providers that stringify nested arrays." },
		],
		description: "PARALLEL mode: [{agent, task, count?, output?, outputMode?, reads?, progress?}, ...]. The literal field name on each item is 'task' (NOT 'prompt', 'title', or 'description').",
	})),
	...
	chain: Type.Optional(Type.Unsafe({
		anyOf: [
			...
			{ type: "string", description: "JSON-encoded array of chain steps; auto-parsed before execution. Provided for providers that stringify nested arrays." },
		],
	...
	chainDir: Type.Optional(Type.String({ description: "Persistent directory for chain artifacts. Default: a user-scoped directory under the Pi ephemeral state area (auto-cleaned after 24h)" })),
	...
	output: Type.Optional(Type.Unsafe({
		...
		description: "Output file for single agent (string), or false to disable. Relative paths resolve against cwd; for ephemeral plans/reports prefer an absolute path under /Users/m/repos/workspace/dotconfig/pi/ephemeral/ with a unique filename.",
	})),
	outputMode: Type.Optional(OutputModeOverride),
	skill: Type.Optional(SkillOverride),
