...
|-------|---------|-------|------------------------|
| `scout` | Fast codebase recon | inherits default | Writes code-context handoff material to the configured Pi ephemeral output path |
| `planner` | Creates implementation plans | inherits default | Writes implementation plans to the configured Pi ephemeral output path |
...
| `researcher` | Web research brief generator | inherits default | Writes research briefs to the configured Pi ephemeral output path |
...
subagent({
  tasks: [
    { agent: "scout", task: "Map auth", output: "/Users/m/repos/workspace/dotconfig/pi/ephemeral/subagent-handoffs/auth-scout-context.md", progress: true },
    { agent: "researcher", task: "Research OAuth best practices", output: "/Users/m/repos/workspace/dotconfig/pi/ephemeral/subagent-handoffs/oauth-research-brief.md" },
...
Avoid duplicate output paths in parallel tasks. Concurrent children should not write to the same file. For ephemeral handoff/scratch outputs, prefer absolute paths under `/Users/m/repos/workspace/dotconfig/pi/ephemeral/` with descriptive unique filenames; do not ask children to write cwd-level `context.md`, `plan.md`, `report.md`, or `progress.md` unless the user explicitly requested that path. For large saved outputs, set `outputMode: "file-only"` together with an `output` path. The parent result then contains only a compact reference like `Output saved to: /abs/report.md (48.2 KB, 2847 lines). Read this file if needed.` instead of the full saved content. Do not use `output: false` for this; `output: false` means no file output. When a task is review-only, say “do not modify project/source files” rather than “do not write files” if you also configured `output`; otherwise the child may treat the output artifact as forbidden. Failed runs and save errors still return inline details for debugging.
