...
## Acceptance Contract`));
	...
	it("top-level parallel progress emits the existing progress instruction style", { skip: !createSubagentExecutor ? "executor not importable" : undefined }, async () => {
		...
		const args = readLastCallArgs();
		const progressMatch = (args.at(-1) ?? "").match(/Update progress at: ([^\n]+)/);
		assert.ok(progressMatch, "expected a progress update path");
		const progressPath = progressMatch[1]!;
		assert.ok(progressPath.includes(`${path.sep}ephemeral${path.sep}`), `expected ephemeral progress path: ${progressPath}`);
