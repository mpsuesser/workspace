...
describe("subagent prompt runtime", () => {
	...
	it("drops signed-thinking subagent assistant turns instead of mutating them", () => {
		const user = { role: "user", content: "Task" };
		const signedSubagentCall = {
			role: "assistant",
			content: [
				{ type: "thinking", thinking: "signed reasoning", thinkingSignature: "opaque-signature" },
				{ type: "text", text: "Launching workers." },
				{ type: "toolCall", id: "toolu_subagent", name: "subagent", arguments: { tasks: [] } },
			],
		};
		const subagentResult = { role: "toolResult", toolCallId: "toolu_subagent", toolName: "subagent", content: "subagent results" };
		const currentTask = { role: "user", content: "Now implement only the assigned fix." };

		assert.deepEqual(
			stripParentOnlySubagentMessages([user, signedSubagentCall, subagentResult, currentTask]),
			[user, currentTask],
		);
	});

	it("drops sibling tool results when a signed-thinking mixed subagent turn is removed", () => {
		const user = { role: "user", content: "Task" };
		const signedMixedCall = {
			role: "assistant",
			content: [
				{ type: "thinking", thinking: "signed reasoning", signature: "provider-signature" },
				{ type: "toolCall", id: "toolu_subagent", name: "subagent", arguments: { agent: "worker" } },
				{ type: "toolCall", id: "toolu_read", name: "read", arguments: { path: "README.md" } },
			],
		};
		const subagentResult = { role: "toolResult", toolCallId: "toolu_subagent", toolName: "subagent", content: "subagent results" };
		const readResult = { role: "toolResult", toolCallId: "toolu_read", toolName: "read", content: "file contents" };
		const nextAssistant = { role: "assistant", content: [{ type: "text", text: "Continuing." }] };

		assert.deepEqual(
			stripParentOnlySubagentMessages([user, signedMixedCall, subagentResult, readResult, nextAssistant]),
			[user, nextAssistant],
		);
	});
