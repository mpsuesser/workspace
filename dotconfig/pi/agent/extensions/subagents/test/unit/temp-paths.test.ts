...
describe("resolvePiEphemeralDir", () => {
	it("places ephemeral state next to the configured pi agent directory", () => {
		assert.equal(
			resolvePiEphemeralDir({ env: { PI_CODING_AGENT_DIR: "/state/pi/agent" } }),
			path.join("/state/pi", "ephemeral"),
		);
	});

	it("allows an explicit ephemeral directory override", () => {
		assert.equal(
			resolvePiEphemeralDir({ env: { PI_SUBAGENT_EPHEMERAL_DIR: "/scratch/pi-ephemeral" } }),
			path.join("/scratch", "pi-ephemeral"),
		);
	});
});

describe("shared temp paths", () => {
	it("anchors shared temp directories under one scoped pi ephemeral root", () => {
		...
		assert.match(path.dirname(TEMP_ROOT_DIR), /(?:^|[\\/])ephemeral$/);
