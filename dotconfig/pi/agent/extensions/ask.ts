...
const AskQuestionSchema = Type.Object({
	...
	shortTitle: Type.Optional(
		Type.String({
			description:
				'Short 1-2 word title for tab display (defaults to number)',
		}),
	),
});
					...
					content: [
						{
							type: 'text',
							text:
								'Error: No questions (or question/options) provided',
						},
					],
				};
			}
	...
	>((tui, theme, _kb, done) =>
	{
		...
		function handleInput(data: string)
		{
			...
			if (matchesKey(data, Key.enter))
			{
				...
				const isLastOption =
					state.optionIndex === allOptions.length - 1;
				if (isLastOption)
				{
					state.editMode = true;
					refresh();
		...
		function renderQuestion(width: number): string[]
		{
			...
			{
				add(
					` ${k('↑↓')}${l(' select')} · ${k('tab')}${l(' next')} · ${
						k('enter')
					}${l(' submit')} · ${k('esc')}${l(' dismiss')}`,
				);
			}
