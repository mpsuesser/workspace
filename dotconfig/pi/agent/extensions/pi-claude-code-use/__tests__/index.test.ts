...
function mockTool(
	name: string,
	sourceOverrides?: { baseDir?: string; path?: string },
)
{
	return {
		name,
...
function createMockPi()
{
	return {
		...
		getCommands: vi.fn(() => []),
		getFlag: vi.fn((_name?: string): boolean | string | undefined =>
			undefined
		),
...
describe('pi-claude-code-use', () =>
{
	...
	it('registers event hooks without overriding the anthropic provider', async () =>
	{
		...
		expect(pi.on).toHaveBeenCalledWith('session_start',
			expect.any(Function));
		expect(pi.on).toHaveBeenCalledWith('before_agent_start',
			expect.any(Function));
		expect(pi.on).toHaveBeenCalledWith('before_provider_request',
			expect.any(Function));
	...
	it('does not call runtime-only APIs during extension factory load', async () =>
	{
		...
		await expect(piClaudeCodeUse(pi as unknown as ExtensionAPI)).resolves
			.toBeUndefined();
	});
	...
	it("replaces 'pi itself' in string-form system prompts", () =>
	{
		...
		expect(result.system).toBe(
			'Pi docs (read about the cli itself and its SDK):',
		);
	});
	...
	it('rewrites text blocks in array system prompts while preserving metadata', () =>
	{
		const result = _test.transformPayload(
			{
				system: [
					...
					},
					{
						type: 'text',
						text:
							'Pi docs (read about pi itself, its SDK, extensions):',
						cache_control: { type: 'ephemeral', ttl: '1h' },
					},
				],
				messages: [],
		...
		expect(result.system).toEqual([
			...
			},
			{
				type: 'text',
				text:
					'Pi docs (read about the cli itself, its SDK, extensions):',
				cache_control: { type: 'ephemeral', ttl: '1h' },
			},
		]);
	});
	...
	it('leaves non-text system blocks untouched', () =>
	{
		...
		expect(result.system).toEqual([{
			type: 'text',
			text: 'about the cli itself',
		}, guardBlock]);
	});

	it("handles multiple occurrences of 'pi itself' in one block", () =>
	{
		expect(_test.rewritePromptText('pi itself and pi itself again')).toBe(
			'the cli itself and the cli itself again',
		);
	});
	...
	it('passes core tools, typed tools, and mcp__-prefixed tools through', () =>
	{
		...
		const toolIds = (result.tools as { name?: string; type?: string }[])
			.map((t) => t.name ?? t.type);
	...
	it('leaves non-tool tool_choice types unchanged', () =>
	{
		...
		expect(_test.remapToolChoice({ type: 'auto' }, surviving)).toEqual({
			type: 'auto',
		});
		expect(_test.remapToolChoice({ type: 'any' }, surviving)).toEqual({
			type: 'any',
		});
	});
	...
	it('applies all transforms together: system rewrite, tool filter, tool_choice, messages', () =>
	{
		const result = _test.transformPayload(
			{
				...
				messages: [{
					role: 'user',
					content: [{ type: 'text', text: 'Search for bugs' }],
				}],
				tools: [
					{
						name: 'Read',
		...
		expect(
			(result.tools as { name?: string; type?: string }[]).map((t) =>
				t.name ?? t.type
			),
		).toEqual([
			'Read',
			...
			_test.isCompanionSource(
				{
					...
					sourceInfo: {
						...
						path:
							'/tmp/node_modules/@benvargas/pi-exa-mcp/extensions/index.ts',
					},
				} as never,
				spec,
			),
			...
			_test.isCompanionSource(
				{
					...
					sourceInfo: {
						...
						path:
							'/worktree/packages/pi-exa-mcp/extensions/index.ts',
					},
				} as never,
				spec,
			),
			...
			_test.isCompanionSource(
				{
					...
					sourceInfo: {
						path:
							'/worktree/node_modules/@benvargas/pi-exa-mcp/extensions/index.ts',
						source: 'test',
						scope: 'user' as const,
						origin: 'package' as const,
					},
			...
			_test.isCompanionSource(
				{
					...
					sourceInfo: {
						path:
							'/worktree/node_modules/@benvargas/pi-exa-mcp-wrapper/extensions/index.ts',
						source: 'test',
						scope: 'user' as const,
						origin: 'package' as const,
					},
			...
			_test.isCompanionSource(
				{
					...
					sourceInfo: {
						...
						baseDir:
							'/home/user/.pi/agent/git/github.com/ben-vargas/pi-packages',
						path:
							'/home/user/.pi/agent/git/github.com/ben-vargas/pi-packages/packages/pi-exa-mcp/extensions/index.ts',
						source: 'test',
						scope: 'user' as const,
						origin: 'package' as const,
					},
	...
	it('preserves user-selected aliases when disabling auto-activation', () =>
	{
		...
		pi.getAllTools.mockReturnValue([
			mockTool('mcp__exa__web_search'),
			mockTool('mcp__firecrawl__scrape'),
		]);
		pi.getActiveTools.mockReturnValue([
			'read',
			'mcp__exa__web_search',
	...
	it('does not forward flag registration to realPi and gates flag access through the capture shim', () =>
	{
		...
		const shim = _test.buildCaptureShim(pi as unknown as ExtensionAPI,
			captured);
		...
		shim.registerFlag('--exa-mcp-tools', {
			description: 'tools',
			type: 'string',
		});
	...
	async function getProviderRequestHandler(
		pi: ReturnType<typeof createMockPi>,
	)
	{
		await piClaudeCodeUse(pi as unknown as ExtensionAPI);
		const hookCall = pi.on.mock.calls.find((c: unknown[]) =>
			c[0] === 'before_provider_request'
		);
		return hookCall?.[1] as (
	...
	it('transforms payload when model is anthropic OAuth', async () =>
	{
		...
		expect((p.tools as { name: string }[]).map((t) => t.name)).toEqual([
			'Read',
		]);
	});
	...
	it('returns undefined for anthropic non-OAuth models', async () =>
	{
		...
		const result = handler({
			payload: { system: 'pi itself', tools: [], messages: [] },
		}, ctx);
	...
	it('registers MCP alias tools from companion extension factories', async () =>
	{
		...
		try
		{
			...
			await _test.registerAliasesForLoadedCompanions(
				pi as unknown as ExtensionAPI,
			);

			expect(pi.registerTool).toHaveBeenCalledWith(
				expect.objectContaining({ name: 'mcp__exa__web_search' }),
			);
		}
		finally
		{
