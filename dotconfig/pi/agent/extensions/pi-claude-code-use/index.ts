...
function rewriteSystemField(system: unknown): unknown
{
	...
	{
		if (
			!isPlainObject(block) || block.type !== 'text'
...
): unknown[]
{
	...
	const result = messages.map((msg) =>
	{
		...
		{
			if (
				!isPlainObject(block) || block.type !== 'tool_use'
...
): Record<string, unknown>
{
	...
	{
		payload.messages = remapMessageToolNames(payload.messages,
			survivingNames);
	}
...
): boolean
{
	...
	return normalized.includes(`/${spec.packageName}/`)
...
): Promise<void>
{
	...
	for (const spec of COMPANIONS)
	{
		...
		{
			if (
				registeredMcpAliases.has(mcpName)
				|| knownNames.has(lower(mcpName))
			) continue;
...
function syncAliasActivation(pi: ExtensionAPI, enableAliases: boolean): void
{
	...
	if (enableAliases)
	{
		...
		{
			if (
				activeLc.has(flat) && allNames.has(mcp)
		...
		if (lastManagedToolList !== undefined)
		{
			...
			for (const alias of autoActivatedAliases)
			{
				...
				const flatName = [...FLAT_TO_MCP.entries()].find(([, mcp]) =>
					mcp === alias
				)?.[0];
				if (
					flatName && lastManaged.has(flatName)
					&& !activeSet.has(flatName)
				)
				{
		...
		const activeRegistered = activeNames.filter((n) =>
			registeredMcpAliases.has(n) && allNames.has(n)
		);
		...
		const preserved = activeRegistered.filter((n) =>
			!autoActivatedAliases.has(n)
		);

		// Build result: non-alias tools + preserved user aliases + desired aliases
		const nonAlias = activeNames.filter((n) =>
			!registeredMcpAliases.has(n)
		);
...
export default async function piClaudeCodeUse(pi: ExtensionAPI): Promise<void>
{
	...
	pi.on('before_agent_start', async (_event, ctx) =>
	{
		...
		const isOAuth = model?.provider === 'anthropic'
