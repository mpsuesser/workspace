...
- Changed the default `web_search` workflow to `none`, so agent-initiated searches return raw results without opening the Glimpse/browser curator unless `workflow: "summary-review"` or `/curator on` is requested.
- Reworked `code_search` to use the active search provider stack with `includeContent` enabled instead of the deprecated Exa MCP `get_code_context_exa` tool. Exa is still preferred, but Perplexity/Gemini can now serve as fallbacks when needed.
- Replaced the `code_search` `maxTokens` argument with `numResults` and kept a compatibility shim for older calls during argument preparation.
