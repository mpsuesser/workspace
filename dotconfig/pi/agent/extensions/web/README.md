...
**Zero Config** — Works out of the box with Exa-backed search (direct API with a key, MCP fallback without one). Or sign into Google in Chrome, Arc, Helium, or Chromium for Gemini Web. Add API keys for Exa, Perplexity, or Gemini API for more control.
...
Works immediately with no API keys — Exa-backed search is available out of the box, with MCP fallback when no Exa API key is configured. For more providers or direct API access, add keys to `~/.pi/web-search.json`:
...
| `workflow`          | `none` (default; skip curator) or `summary-review` (open curator and auto-generate a summary draft after search completion) |

### code_search

Search for code examples, documentation, and API references. Prefers Exa and falls back to other available search providers when needed. Uses the same provider stack as `web_search`, but formats results for technical docs/examples and enables inline content extraction for better excerpts.

```typescript
code_search({ query: "React useEffect cleanup pattern" });
code_search({ query: "Express middleware error handling", numResults: 6 });
```

| Parameter    | Description                                               |
| ------------ | --------------------------------------------------------- |
| `query`      | Programming question, API, library, or debugging topic    |
| `numResults` | Maximum number of sources to return (default: 5, max: 10) |
...
code_search(query)
  → Provider search stack with inline content enabled
  → Prefer Exa → fallback to Perplexity / Gemini
  → Return technical docs/examples formatted as numbered sources with excerpts
...
| `code-search.ts`        | Code/docs search via the shared provider stack with formatted technical excerpts              |
