...
interface BrowserConfig {
	...
	/**
	 * Optional sibling keychain item owned by the `security` tool itself. When
	 * set, `readBrowserPassword` tries this service first. Chromium-family
	 * browsers write their Safe Storage item with an ACL + partition list that
	 * only permit the browser, so every read via `security find-generic-password`
	 * triggers an interactive keychain prompt. By caching the decrypted
	 * password into a `security`-owned item once, subsequent reads are silent.
	 *
	 * Bootstrap (one-time, triggers one prompt from the browser item):
	 *   KEY=$(security find-generic-password -w -s "Chrome Safe Storage" -a Chrome)
	 *   security add-generic-password -U \
	 *     -a Chrome -s "Chrome Safe Storage Cache" \
	 *     -D "application password" \
	 *     -j "pi-web cache of Chrome Safe Storage key" \
	 *     -w "$KEY"
	 *   unset KEY
	 *
	 * If the browser rotates its key (rare), decryption of cookies yields zero
	 * results; we detect that, delete the cache, and fall back to the real
	 * item (one prompt) before re-seeding.
	 */
	keychainCacheService?: string;
...
const MACOS_BROWSER_CONFIGS: BrowserConfig[] = [
	...
	},
	{
		...
		keychainCacheService: "Chrome Safe Storage Cache",
	},
	{
		name: "Arc",
...
export async function getGoogleCookies(options?: {
	...
	for (const config of configs) {
		...
		let attempt = await readBrowserPassword(config, currentPlatform, {
			allowCache: true,
		});
		if (!attempt.password) {
		...
		try {
			...
			let result = decodeCookies(rows, attempt.password, currentPlatform, stripHash);

			// Stale-cache detection: the cache yielded a key that decrypted nothing,
			// yet there were encrypted rows available. Almost certainly the browser
			// rotated its Safe Storage key. Invalidate cache and retry with the
			// real (prompting) keychain item.
			const hadEncryptedRows = rows.some((row) => row.encrypted_value instanceof Uint8Array);
			if (
				attempt.fromCache &&
				hadEncryptedRows &&
				result.decryptedCount === 0 &&
				config.keychainCacheService &&
				config.keychainAccount
			) {
				warnings.push(
					`${config.name} cached key failed to decrypt; refreshing from keychain`,
				);
				await deleteKeychainPassword(config.keychainAccount, config.keychainCacheService);
				attempt = await readBrowserPassword(config, currentPlatform, {
					allowCache: false,
				});
				if (attempt.password) {
					result = decodeCookies(rows, attempt.password, currentPlatform, stripHash);
				}
			}

			if (
				options?.requiredCookies?.length &&
				!options.requiredCookies.every((name) => Boolean(result.cookies[name]))
			) {
				continue;
			}
...
function decodeCookies(
	...
	stripHash: boolean,
): { cookies: CookieMap; decryptedCount: number } {
	...
	for (const row of rows) {
		...
		if (!value) {
			...
			if (encrypted instanceof Uint8Array) {
				const decrypted = decryptCookieValue(encrypted, key, stripHash);
				if (decrypted !== null) {
					decryptedCount++;
					value = decrypted;
				}
...
): Promise<{ password: string | null; fromCache: boolean }> {
	if (currentPlatform === "darwin") {
		if (!config.keychainAccount || !config.keychainService) {
			return { password: null, fromCache: false };
		}
		if (opts.allowCache && config.keychainCacheService) {
			const cached = await readKeychainPassword(
				config.keychainAccount,
				config.keychainCacheService,
			);
			if (cached) return { password: cached, fromCache: true };
		}
		const fresh = await readKeychainPassword(config.keychainAccount, config.keychainService);
		if (fresh && config.keychainCacheService) {
			await writeKeychainPassword(config.keychainAccount, config.keychainCacheService, fresh);
		}
		return { password: fresh, fromCache: false };
	}
	if (currentPlatform === "linux") {
		return { password: await readLinuxPassword(config.secretToolApp), fromCache: false };
	}
	return { password: null, fromCache: false };
}
...
function writeKeychainPassword(account: string, service: string, password: string): Promise<void> {
	return new Promise((resolve) => {
		execFile(
			"security",
			[
				...
				"-j",
				"pi-web cache of browser Safe Storage key",
				"-w",
				password,
			],
			{ timeout: 5000 },
...
function deleteKeychainPassword(account: string, service: string): Promise<void> {
	return new Promise((resolve) => {
		execFile(
			"security",
