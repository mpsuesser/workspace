...
import { Box, Text, truncateToWidth, type KeyId } from "@mariozechner/pi-tui";
...
type WebToolResult = AgentToolResult<ToolDetails>;
...
const DEFAULT_SHORTCUTS: { curate: KeyId; activity: KeyId } = {
	curate: "ctrl+shift+s",
	activity: "ctrl+shift+w",
};
const DEFAULT_WEB_SEARCH_WORKFLOW: WebSearchWorkflow = "none";
...
export function resolveWorkflow(input: unknown, hasUI: boolean): WebSearchWorkflow {
	...
	if (typeof input !== "string") return DEFAULT_WEB_SEARCH_WORKFLOW;
	const normalized = input.trim().toLowerCase();
	if (normalized === "summary-review") return "summary-review";
	if (normalized === "none") return "none";
	return DEFAULT_WEB_SEARCH_WORKFLOW;
}
...
/**
 * Coerce a value that should be `string[]` but may arrive from the LLM as:
 *   - a JSON-encoded array string: '["a","b"]'
 *   - a plain string (single item): "a"
 *   - a comma-separated string: "a, b, c"
 *   - an actual array
 * Returns the coerced array, or the original value if no coercion applies.
 * Mis-typed arrays from models are a common tool-call failure; prefer to
 * accept and normalize rather than reject outright.
 */
export function coerceStringArrayArg(value: unknown): unknown {
...
/**
 * Repair common LLM mistakes on a tool-args object before schema validation:
 * if the plural (array) field arrived as a string, coerce it. If the singular
 * field arrived as an array, move the first item over.
 */
export function coerceArrayFieldArgs(args: unknown, singular: string, plural: string): unknown {
	if (!args || typeof args !== "object" || Array.isArray(args)) return args;
	const input = args as Record<string, unknown>;
	const out: Record<string, unknown> = { ...input };
	if (out[plural] !== undefined && !Array.isArray(out[plural])) {
		out[plural] = coerceStringArrayArg(out[plural]);
	}
	if (Array.isArray(out[singular])) {
		const arr = out[singular] as unknown[];
		if (out[plural] === undefined) out[plural] = arr;
		out[singular] = typeof arr[0] === "string" ? arr[0] : undefined;
	}
	// Also coerce domainFilter if present as a string
	if (out.domainFilter !== undefined && !Array.isArray(out.domainFilter)) {
		out.domainFilter = coerceStringArrayArg(out.domainFilter);
	}
	return out;
}
...
interface PendingCurate {
	...
	abortSearches: () => void;
	finish: (value: WebToolResult) => void;
...
function formatEntryLine(entry: ActivityEntry, theme: ExtensionContext["ui"]["theme"]): string {
...
export default function (pi: ExtensionAPI) {
	...
	const curateKey =
		(initConfig.shortcuts?.curate as KeyId | undefined) ?? DEFAULT_SHORTCUTS.curate;
	const activityKey =
		(initConfig.shortcuts?.activity as KeyId | undefined) ?? DEFAULT_SHORTCUTS.activity;
	...
	function buildCurationCancelledReturn(reason: "user" | "stale"): WebToolResult {
	...
	pi.registerTool({
		name: "web_search",
		label: "Web Search",
		description: `Search the web using Perplexity AI, Exa, or Gemini. Returns an AI-synthesized answer with source citations. For comprehensive research, prefer queries (plural) with 2-4 varied angles over a single query — each query gets its own synthesized answer, so varying phrasing and scope gives much broader coverage. When includeContent is true, full page content is fetched in the background. By default, searches return raw results without opening the curator; set workflow to "summary-review" to open the interactive curator with an auto-generated summary draft. Provider auto-selects: Exa (direct API with key, MCP fallback without), else Perplexity (needs key), else Gemini API (needs key), else Gemini Web (needs a supported Chromium-based browser login).`,
		promptSnippet:
		...
		parameters: Type.Object({
			...
			workflow: Type.Optional(
				StringEnum(["none", "summary-review"], {
					description:
						"Search workflow mode: none = no curator (default), summary-review = open curator with auto summary draft",
				}),
			),
		}),
		prepareArguments(args) {
			return coerceArrayFieldArgs(args, "query", "queries") as never;
		},

		async execute(_toolCallId, params, signal, onUpdate, ctx): Promise<WebToolResult> {
			...
			if (shouldCurate) {
				...
				let resolvePromise: (value: WebToolResult) => void = () => {};
				const promise = new Promise<WebToolResult>((resolve) => {
				...
				const finish = (value: WebToolResult) => {
					if (cancelled) return;
					cancelled = true;
					pc.abortSearches();
	...
	pi.registerTool({
		...
		label: "Code Search",
		description:
			"Search for code examples, documentation, and API references. Returns relevant docs, examples, and implementation notes from official documentation, GitHub, Stack Overflow, and other technical sources.",
		promptSnippet:
		...
		prepareArguments(args): CodeSearchParams {
			if (!args || typeof args !== "object" || Array.isArray(args))
				return args as unknown as CodeSearchParams;
			const input = args as Record<string, unknown>;
			if (input.numResults !== undefined || input.maxTokens === undefined) {
				return input as unknown as CodeSearchParams;
			}
			const numResults = legacyMaxTokensToNumResults(input.maxTokens);
			if (numResults === undefined) {
				return input as unknown as CodeSearchParams;
			}
			const { maxTokens: _maxTokens, ...rest } = input;
			return { ...rest, numResults } as CodeSearchParams;
		},
		...
		renderResult(result, { expanded }, theme) {
			...
			const resultCount = details?.resultCount ?? details?.numResults ?? 0;
			const sourceLabel = resultCount === 1 ? "source" : "sources";
			const providerLabel = details?.provider ? ` via ${details.provider}` : "";
			const summary =
				theme.fg("success", `${resultCount} ${sourceLabel}`) +
	...
	pi.registerTool({
		...
		prepareArguments(args) {
			return coerceArrayFieldArgs(args, "url", "urls") as never;
		},

		async execute(_toolCallId, params, signal, onUpdate): Promise<WebToolResult> {
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params): Promise<WebToolResult> {
	...
	pi.registerCommand("websearch", {
		...
		handler: async (args, ctx) => {
			...
			function sendFollowUpFromReturn(payload: ReturnType<typeof buildSearchReturn>): void {
				pi.sendMessage(
					{
