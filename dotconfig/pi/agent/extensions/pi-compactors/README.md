...
[![npm](https://img.shields.io/npm/v/@sting8k/pi-compactors)](https://www.npmjs.com/package/@sting8k/pi-compactors)

Algorithmic conversation compactors for [Pi](https://github.com/badlogic/pi-mono). No LLM calls — includes `compact-vcc` and `compact-meat` modes for deterministic compaction.

Inspired by [VCC](https://github.com/lllyasviel/VCC) **(View-oriented Conversation Compiler)**.

## Why pi-compactors

|  | Pi default | pi-compactors |
|---|---|---|
...
- **`/compact-vcc-recall`** — slash command to search history directly, results shown as collapsible message and auto-fed to agent as context
- **Fallback cut** — still works when Pi core returns nothing to summarize
- **Redaction** — strips passwords, API keys, secrets
- **`/compact-vcc`** — manual compaction on demand with structured summary + compressed transcript
- **`/compact-meat`** — manual compaction that keeps only user messages and assistant prose

## Install

```bash
pi install npm:@sting8k/pi-compactors
```

Or from GitHub:

```bash
pi install https://github.com/sting8k/pi-compactors
```

Or try without installing:

```bash
pi -e https://github.com/sting8k/pi-compactors
```

## Usage

Once installed, pi-compactors registers a `session_before_compact` hook.

- When Pi triggers a compaction, pi-compactors supplies the summary.
- To trigger structured compaction manually, run `/compact-vcc`.
- To trigger prose-only compaction manually, run `/compact-meat`.
- To search older history after compaction, use `vcc_recall`.
- To search and feed results to agent yourself, run `/compact-vcc-recall <query> [page:N]`.
  - Tip: type `/recall` and Pi will autocomplete to `/compact-vcc-recall`.

### Compacted message structure

`/compact-vcc` produces the structured format below.

`/compact-meat` produces a much simpler transcript that keeps only `[user]` and `[assistant]` prose sections, with no tool calls, tool results, or tool errors.
...
Debug logging is off by default. Enable it in `~/.pi/agent/pi-compactors-config.json`:

```json
{ "debug": true }
```

When enabled, each compaction writes detailed info to `/tmp/pi-compactors-debug.json` — message counts, cut boundary, summary preview, sections.
