...
import { compileMeat } from "../core/meat";
import type { PiCompactorsCompactionDetails } from "../details";

const CONFIG_PATH = join(homedir(), ".pi", "agent", "pi-compactors-config.json");
const LEGACY_CONFIG_PATH = join(homedir(), ".pi", "agent", "pi-vcc-config.json");
export const COMPACT_VCC_INSTRUCTION = "__compact_vcc__";
export const COMPACT_MEAT_INSTRUCTION = "__compact_meat__";
const LEGACY_VCC_INSTRUCTION = "__pi_vcc__";
const LEGACY_MEAT_INSTRUCTION = "__pi_vcc_meat__";
...
export interface PiCompactorsConfig {
  debug?: boolean;
}

const loadConfig = (): PiCompactorsConfig => {
...
const dbg = (config: PiCompactorsConfig, data: Record<string, unknown>) => {
  if (!config.debug) return;
  try { writeFileSync("/tmp/pi-compactors-debug.json", JSON.stringify(data, null, 2)); } catch {}
};
...
export const registerBeforeCompactHook = (pi: ExtensionAPI) => {
  pi.on("session_before_compact", (event) => {
    ...
    const mode = customInstructions === COMPACT_VCC_INSTRUCTION || customInstructions === LEGACY_VCC_INSTRUCTION
      ? "compact-vcc"
      : customInstructions === COMPACT_MEAT_INSTRUCTION || customInstructions === LEGACY_MEAT_INSTRUCTION
        ? "compact-meat"
        : null;
    if (!mode) return;
    ...
    const summary = mode === "compact-meat"
      ? compileMeat({
          messages,
    ...
    const details: PiCompactorsCompactionDetails = {
